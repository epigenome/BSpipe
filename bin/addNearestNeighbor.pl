...
## Given a locus, add the nearest feature in both strands
...
sub load
{
	...
	while (<$in>)
	{
		...
		{
			push(@{$features{$a[$iRef]}{$i}}, \@v);
			#print "$a[$iRef] $i\n";
		}

		$features{$a[$iRef]}{S} = int(($a[$iStart]-$allow)/$break) if !exists $features{$a[$iRef]}{S} || $features{$a[$iRef]}{S} > int(($a[$iStart]-$allow)/$break);
		$features{$a[$iRef]}{E} = int(($a[$iEnd  ]+$allow)/$break) if !exists $features{$a[$iRef]}{E} || $features{$a[$iRef]}{E} < int(($a[$iEnd  ]+$allow)/$break);
		}
...
sub search
{
	my ($id, $start, $end) = @_;
	print "$id $start $end\n" if $verbose;
	return ($Null) if !exists $features{$id}{S};

	my (%done);
	my @ret1 = search2($features{$id}, int($start/$break)  , $features{$id}{E},  1, $start, $end, \%done);
	my @ret2 = search2($features{$id}, int($start/$break)-1, $features{$id}{S}, -1, $start, $end, \%done);
...
sub checkOptions
{
	...
	if ($helpFlag || (!$featureFile && !$inFile) || !$name || !$iRef || !$iStart || !$iEnd)
	{
