...
my $rscript = "/app/redhat6/R/R-3.6.1/bin/Rscript " . dirname($0) . "/chisquare.r";
...
sub process
{
	...
	print $out "#Ref\tStart\tEnd\tNreads\tNsites\tNpats\tCommonSites\tCommonPats\tEntropy\tChiSquare\n";
...
{
	my ($ref, $start) = @_;
	my (%poss, %comPoss, %comPats);
	map {$poss{$_}++} @{$wins{$ref}{$start}[0][0]};
	map {$poss{$_}++} @{$wins{$ref}{$start}[1][0]};
	map {$comPoss{$_}++ if $poss{$_} > 1} keys %poss;

	modify_pattern($wins{$ref}{$start}[0][0], \%comPoss, $wins{$ref}{$start}[0][1]) if $wins{$ref}{$start}[0][4] != keys %comPoss;
	modify_pattern($wins{$ref}{$start}[1][0], \%comPoss, $wins{$ref}{$start}[1][1]) if $wins{$ref}{$start}[1][4] != keys %comPoss;

	my $entropy = relative_entropy(@{$wins{$ref}{$start}[0]}[1,2], @{$wins{$ref}{$start}[1]}[1,2], \%comPats);
	...
	print $out join("\t", $ref, $start, $start+$length,
		...
		scalar keys %comPoss, scalar keys %comPats);

	if (keys %comPats)
	{
		my $chisq = chisquare_test($wins{$ref}{$start}[0][1], $wins{$ref}{$start}[1][1], \%comPats);
		die "Error on R for $ref $start\n" if $chisq < 0;
		print $out "\t", sprintf("%.3f", $entropy), "\t", $chisq, "\n";
	}
	else
	{
		print $out "\t", '-', "\t", '-', "\n";
...
sub chisquare_test
{
	my ($rpat1, $rpat2, $rcom) = @_;
	my $pval;

	my $pid = open2(my $childOut, my $childIn, $rscript);
	#print "pid: $pid\n";

	print $childIn join("\t", keys %$rcom), "\n";
	print $childIn join("\t", map {$rpat1->{$_}} keys %$rcom), "\n";
	print $childIn join("\t", map {$rpat2->{$_}} keys %$rcom), "\n";
